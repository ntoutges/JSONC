...
function parseAll(jsonString) {
    ...
    while (index < tokens.length) {
        ...
        if (token.type == "COMMENT_BLOCK" || token.type == "COMMENT_INLINE") {
            if (token.type == "COMMENT_BLOCK") { // block keys always come AFTER
                ...
                if (index < tokens.length && tokens[index].type == "LINE_END")
                    index++; // skip next newline
            }
            else { // inline comment that refers to its own line
                const keysCopy = keys.slice(); // make copy
                if (keys[keys.length - 1] === null)
                    keysCopy.pop(); // cut off last element (only one that can be uninitalized)
        ...
        if (isNextKey && (token.type == "RIGHT_BRACKET" || token.type == "RIGHT_BRACE"))
            isNextKey = false;
        if (isNextKey) {
            ...
            if (keys[keys.length - 1] === null)
                keysCopy.pop(); // cut off last element (only one that can be uninitalized)
            for (const comment of commentStack) {
                comment.reference = keysCopy;
            }
        ...
        switch (token.type) {
            ...
            case "RIGHT_BRACKET":
            case "RIGHT_BRACE": {
                if ((token.type == "RIGHT_BRACKET" && stackItem.type != "array")
                ...
                if (Array.isArray(stackItem.object) && stackItem.type != "array")
                ...
                if (stackItem.type != "array")
                    isNextKey = true; // object requiring key
                break;
            }
            case "REFERENCE":
            case "NUMBER":
    ...
    for (const ref of selfRefs) {
        ...
        for (const i in keys) {
            const subkey = keys[i];
            if (!head.hasOwnProperty(subkey))
                throw new Error(`Reference *[${keys.join(",")}] could not be reached`);
            if (typeof head != "object" || head === null)
                throw new Error(`Reference *[${keys.join(",")}] refers to non-iterable object`);
            ...
            case '/':
                if (jsonString[index + 1] === '/') {
                    ...
                    if (tokens.length >= 2 && tokens[tokens.length - 1].type == "LINE_END" && tokens[tokens.length - 2].type == "COMMENT_INLINE") {
                        ...
                        tokens.push({ type: "COMMENT_INLINE", value: comment.trim() });
                }
            ...
            default:
                if (/\d/.test(char)) {
                    ...
                    if (jsonString[index] == "n") { // signifies BigInt
                        ...
                        tokens.push({ type: "NUMBER", value: parseInt(numberValue) });
                }
                ...
                switch (typeof token.value) {
                    ...
                    case "object": // null
                        if (token.value === null)
                            ...
                            str += `${key}\"[Object object]\"`; // invalid
                        break;
                }
                break;
            case "reference":
                str += `${key}: ${"*[" + token.value.map(val => JSON.stringify(val)).join(",") + "]"}`;
                break;
        }
...
function isCommentMultiline(comment) {
    if (comment.inline === false)
        return true; // block comment, these WILL (eventually) take up multiple lines
    return comment.text.indexOf("\n") != -1;
}
function getCommentText(comment, tabs = "", lastChar = "") {
